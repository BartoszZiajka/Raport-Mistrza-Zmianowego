...
public class AddReportController implements Initializable {
    ...
    @FXML
    public MaskField porterToHourMaskField;
    @FXML
...
//        form validating
        ...
        if (porterUntilTextField.getText().isEmpty() || porterFromTextField.getText().isEmpty() || porterToHourMaskField.getText().isEmpty() || porterFromHourMaskField.getText().isEmpty()) {
...
//        check if record with this data exist in database
        if (!reportConnector.isDuplicate(String.valueOf(datePicker.getValue()), workingHoursComboBox.getValue())) {
            reportConnector.createNewRecord(datePicker.getValue(), workingHoursComboBox.getValue(), shiftMasterTextField.getText(), porterToHourMaskField.getText(), porterFromHourMaskField.getText(), shiftReportTextArea.getText(), standbyZasoleTextField.getText(), standbyZasoleTextFieldSecondShift.getText(), standbyZaborzeTextField.getText(), standbyZaborzeTextFieldSecondShift.getText(), standbyHydroforniaTextField.getText(), standbyHydroforniaTextFieldSecondShift.getText(), standbyPrzepompowniaTextField.getText(), standbyPrzepompowniaTextFieldSecondShift.getText(), pw15ZasoleTextField.getText(), c15ZasoleTextField.getText(), pw20ZasoleTextField.getText(), c20ZasoleTextField.getText(), przepMinZasoleTextField.getText(), przepMaxZasoleTextField.getText(), pw15ZaborzeTextField.getText(), c15ZaborzeTextField.getText(), pw20ZaborzeTextField.getText(), c20ZaborzeTextField.getText(), przepMinZaborzeTextField.getText(), przepMaxZaborzeTextField.getText(), pw15HydroforniaTextField.getText(), c15HydroforniaTextField.getText(), pw20HydroforniaTextField.getText(), c20HydroforniaTextField.getText(), odczytChelmekTextField.getText(), zuzycieChelmekTextField.getText(), overtimesTableView.getItems(), porterUntilTextField.getText(), porterFromTextField.getText());
//            show confirmation message
            showAlert(Alert.AlertType.CONFIRMATION, "Sukces!", "Raport dodany pomyślnie");
        } else {
            showAlert(Alert.AlertType.ERROR, "Ups!", "Istnieje już raport z taką datą! Zmień datę lub przejdź do sekcji \"Edycja\" i edytuj raport");
        }
    }
...
//        create new alert
    ...
    private void setupMaskFields() {
        porterToHourMaskField.focusedProperty().addListener((obs, oldVal, newVal) -> porterToHourMaskField.setMask("DD:DD"));
    ...
    private void setupComboBoxes() {
        for (String pracownik : overtimesConnector.getEmployees())
